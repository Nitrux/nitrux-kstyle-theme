...
void WindowManager::registerWidget(QWidget *widget)
{
    ...
    if (isBlackListed(widget) || isDraggable(widget)) {
        /*
...
{
    if (!object || !event || event->type() != QEvent::MouseButtonPress) {
        return false;
    }
    ...
    // cast to widget before changing drag state
    ...
    // retrieve widget child at event position
    ...
    // check lock only after all validation succeeds
    if (isLocked()) {
        return false;
    }
    setLocked(true);
