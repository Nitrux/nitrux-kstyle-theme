...
#include "nitruxanimations.h"
#include "nitruxmetrics.h"

#include <KConfigWatcher>
#include "nitruxmnemonics.h"
#include "nitruxpropertynames.h"
#include "nitruxsplitterproxy.h"
#include "nitruxstyleconfigdata.h"
#include "nitruxtoolsareamanager.h"
#include "nitruxwindowmanager.h"
...
#include "nitrux_logging.h"

constexpr auto HighlightColor = QPalette::Accent;

namespace NitruxPrivate
{
...
private:
    ...
    Nitrux::WeakPointer<const QWidget> _tabBar;
};
...
public:
    ...
    explicit ComboBoxItemDelegate(QAbstractItemView *parent)
        ...
        , _itemMargin(Nitrux::Metrics::ComboBox_ItemMarginWidth)
    {
    }
...
private:
    //* proxy
    Nitrux::WeakPointer<QAbstractItemDelegate> _proxy;
...
Style::Style()
    ...
    , _tabBarData(std::make_unique<NitruxPrivate::TabBarData>())
#if NITRUX_HAVE_KSTYLE
...
#endif
{
    ...
    _configWatcher = KConfigWatcher::create(StyleConfigData::self()->sharedConfig());
    connect(_configWatcher.data(), &KConfigWatcher::configChanged, this, &Style::configurationChanged);
    ...
    if (qobject_cast<QScrollBar *>(widget)) {
        ...
        if (!hasParent(widget, "QWebView")) {
            ...
            if (itemView && itemView->itemDelegate() && itemView->itemDelegate()->inherits("QComboBoxDelegate")) {
                itemView->setItemDelegate(new NitruxPrivate::ComboBoxItemDelegate(itemView));
            }
        }
...
int Style::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget, QStyleHintReturn *returnData) const
{
    ...
    case SH_Widget_Animation_Duration:
        return StyleConfigData::animationsEnabled() ? qBound(0, StyleConfigData::animationsDuration(), 60000) : 0;
...
bool Style::drawWidgetPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (mw && mw == mw->window()) {
        ...
        if (auto vLayout = qobject_cast<QVBoxLayout *>(widget->layout())) {
            ...
            if (rect.height() > 0) {
                ...
                // Add the layout’s top contents margin; no separator is painted here.
                rect.setHeight(rect.height() + vLayout->contentsMargins().top());
    ...
    } else if (widget && widget->inherits("KMultiTabBar") && hasParent(widget, "KateMDI::Sidebar")) {
        // KateMDI owns this tab bar; its pane boundary is intentionally flat.
        return true;
...
void Style::loadGlobalAnimationSettings()
{
    ...
    const double rawFactor = cg.readEntry("AnimationDurationFactor", StyleConfigData::animationsDuration() / 100.0);
    const double factor = std::isfinite(rawFactor) ? qBound(0.0, rawFactor, 600.0) : 1.0;
    const int animationsDuration = static_cast<int>(factor * 100.0);
...
{
    // Apply the shared MauiKit radius before painting any widget.
    _helper->setBorderRadius(StyleConfigData::borderRadius());
    _helper->loadConfig();
...
QRect Style::progressBarGrooveRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    const bool horizontal(NitruxPrivate::isProgressBarHorizontal(progressBarOption));
    const int thickness = busy ? Metrics::ProgressBar_BusyThickness : Metrics::ProgressBar_Thickness;
    ...
    if (textVisible && !busy && horizontal) {
        ...
        rect = centerRect(rect, rect.width(), thickness);

    } else if (horizontal) {
        rect = centerRect(rect, rect.width(), thickness);

    } else {
        rect = centerRect(rect, thickness, rect.height());
    }
...
QRect Style::progressBarContentsRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    const qreal position = qBound<qreal>(0.0, qreal(progress) / qreal(steps), 1.0);
...
QRect Style::tabWidgetTabPaneRect(const QStyleOption *option, const QWidget *) const
{
    ...
    const int frameGap = Metrics::TabBar_FrameGap;

    auto rect(option->rect);
    switch (tabOption->shape) {
    case QTabBar::RoundedNorth:
    case QTabBar::TriangularNorth:
        rect.adjust(0, tabBarSize.height() + frameGap, 0, 0);
        break;

    case QTabBar::RoundedSouth:
    case QTabBar::TriangularSouth:
        rect.adjust(0, 0, 0, -tabBarSize.height() - frameGap);
        break;

    case QTabBar::RoundedWest:
    case QTabBar::TriangularWest:
        rect.adjust(tabBarSize.width() + frameGap, 0, 0, 0);
        break;

    case QTabBar::RoundedEast:
    case QTabBar::TriangularEast:
        rect.adjust(0, 0, -tabBarSize.width() - frameGap, 0);
        break;
...
QRect Style::toolButtonSubControlRect(const QStyleOptionComplex *option, SubControl subControl, const QWidget *widget) const
{
    ...
    const auto menuStyle = NitruxPrivate::toolButtonMenuArrowStyle(toolButtonOption);
    ...
    case SC_ToolButtonMenu: {
        // check features
        if (menuStyle == NitruxPrivate::ToolButtonMenuArrowStyle::None) {
            return QRect();
        }

        auto menuRect(rect);
        if (menuStyle == NitruxPrivate::ToolButtonMenuArrowStyle::InlineSmall) {
    ...
    case SC_ToolButton: {
        if (menuStyle == NitruxPrivate::ToolButtonMenuArrowStyle::SubControl) {
...
QSize Style::toolButtonSizeFromContents(const QStyleOption *option, const QSize &contentsSize, [[maybe_unused]] const QWidget *widget) const
{
    ...
    const auto menuStyle = NitruxPrivate::toolButtonMenuArrowStyle(toolButtonOption);
    if (menuStyle == NitruxPrivate::ToolButtonMenuArrowStyle::InlineLarge) {
        size.rwidth() += Metrics::MenuButton_IndicatorWidth;
    }
...
QSize Style::menuItemSizeFromContents(const QStyleOption *option, const QSize &contentsSize, const QWidget *widget) const
{
    ...
    case QStyleOptionMenuItem::SubMenu: {
        ...
        if (showIconsInMenuItems()) {
            iconWidth = menuItemOption->maxIconWidth;
        }
...
QSize Style::progressBarSizeFromContents(const QStyleOption *option, const QSize &contentsSize, const QWidget *) const
{
    ...
    const bool horizontal(NitruxPrivate::isProgressBarHorizontal(progressBarOption));
    const int thickness = progressBarOption->minimum == 0 && progressBarOption->maximum == 0
        ? Metrics::ProgressBar_BusyThickness
        : Metrics::ProgressBar_Thickness;
    ...
    if (horizontal) {
        ...
        size.setWidth(qMax(size.width(), thickness));
        size.setHeight(qMax(size.height(), thickness));
        if (textVisible) {
...
bool Style::drawFrameLineEditPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // Editable combo boxes use the same filled, borderless surface as
    // ordinary combo boxes.
    if (widget && qobject_cast<const QComboBox *>(widget)) {
        QHash<QByteArray, bool> stateProperties;
        stateProperties["enabled"] = option->state & State_Enabled;
        stateProperties["hovered"] = option->state & State_MouseOver;
        stateProperties["down"] = option->state & (State_Sunken | State_On);
        stateProperties["flat"] = false;
        _helper->renderButtonFrame(painter, rect, palette, stateProperties, AnimationData::OpacityInvalid, AnimationData::OpacityInvalid);
        return true;
    }
...
{
    // draw frames for expanded toolbars
...
bool Style::drawFrameTabWidgetPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    case QTabBar::RoundedNorth:
    case QTabBar::TriangularNorth:
        if (tabBarSize.width() >= rect.width() - 2 * _helper->frameRadius()) {
            corners &= ~CornersTop;
        }
        if (tabBarRect.left() < rect.left() + _helper->frameRadius()) {
            corners &= ~CornerTopLeft;
        }
        if (tabBarRect.right() > rect.right() - _helper->frameRadius()) {
    ...
    case QTabBar::RoundedWest:
    case QTabBar::TriangularWest:
        if (tabBarSize.height() >= rect.height() - 2 * _helper->frameRadius()) {
            corners &= ~CornersLeft;
        }
        if (tabBarRect.top() < rect.top() + _helper->frameRadius()) {
            corners &= ~CornerTopLeft;
        }
        if (tabBarRect.bottom() > rect.bottom() - _helper->frameRadius()) {
            corners &= ~CornerBottomLeft;
        }
        break;
...
bool Style::drawFrameTabBarBasePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    // MauiKit-style tab bars do not draw a separator or focus frame here.
    Q_UNUSED(painter);
...
bool Style::drawIndicatorArrowPrimitive(ArrowOrientation orientation, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (inTabBar) {
        ...
        if (flat && menuStyle != NitruxPrivate::ToolButtonMenuArrowStyle::None) {
            if (sunken && !mouseOver) {
        ...
        color = (enabled && (state & (State_Sunken | State_On)))
            ? palette.color(QPalette::HighlightedText)
            : _helper->arrowColor(palette, QPalette::WindowText);
    }
...
bool Style::drawPanelButtonToolPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (menuStyle == NitruxPrivate::ToolButtonMenuArrowStyle::SubControl) {
        ...
        baseRect.adjust(0, 0, _helper->frameRadius(), 0);
...
bool Style::drawTabBarPanelButtonToolPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // The primitive can be called directly, so validate the parent widget
...
bool Style::drawPanelMenuPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const auto &palette(option->palette);
    const bool hasAlpha(_helper->hasAlphaChannel(widget));
    const int menuOpacity = qBound(0, StyleConfigData::menuOpacity(), 100);
    ...
    if (menuOpacity < 100) {
        ...
        background.setAlphaF(menuOpacity / 100.0);
    ...
    if (treeItemView && treeItemView->header()) {
        ...
        if (treeItemView->columnViewportPosition(prevColumn) != -1) {
            if (viewItemPosition != QStyleOptionViewItem::Beginning && viewItemPosition != QStyleOptionViewItem::OnlyOne
                && treeItemView->columnWidth(prevColumn) < _helper->frameRadius()) {
                rect.setX(rect.x() + _helper->frameRadius());
            }
        }
        if (treeItemView->columnViewportPosition(thisColumn) != -1) {
            if (treeItemView->columnWidth(thisColumn) < _helper->frameRadius()) {
                if (viewItemPosition == QStyleOptionViewItem::Beginning) {
                    rect.setWidth(rect.width() + _helper->frameRadius());
                } else {
                    rect.setX(rect.x() - _helper->frameRadius());
                }
            }
        ...
        if (treeItemView->columnViewportPosition(nextColumn) != -1) {
            if (viewItemPosition != QStyleOptionViewItem::End && treeItemView->columnWidth(nextColumn) < _helper->frameRadius()) {
                rect.setWidth(rect.width() - _helper->frameRadius());
            }
        }
    }
...
bool Style::drawIndicatorButtonDropDownPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    baseRect.adjust(-_helper->frameRadius(), 0, 0, 0);
    baseRect = visualRect(option, baseRect);
    ...
    QRectF frameRect = _helper->strokedRect(baseRect);

    // also render separator
    if (!flat || activeFocus || hovered || down || checked || penAnimation != AnimationData::OpacityInvalid) {
        painter->setBrush(Qt::NoBrush);
        if (option->direction == Qt::RightToLeft) {
            QRectF separatorRect = frameRect.adjusted(0, 0, -_helper->frameRadius() , 0);
            painter->drawLine(separatorRect.topRight(), separatorRect.bottomRight());
        } else {
            QRectF separatorRect = frameRect.adjusted(_helper->frameRadius(), 0, 0, 0);
...
bool Style::drawDockWidgetResizeHandlePrimitive(const QStyleOption *, QPainter *, const QWidget *) const
{
    // Keep the resize hit area, but do not paint a separator between dock panes
    // and the main view.
    return true;
}
...
{
    if (widget && !widget->property("_nitrux_statusbar_separator").toBool() && widget->parent() && !widget->parent()->inherits("QMainWindow")) {
        return true;
    }
    auto rect(option->rect);
    const auto color(_helper->separatorColor(option->palette));
...
bool Style::drawPushButtonLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // color role
    const QPalette::ColorRole textRole = sunken && enabled
        ? QPalette::HighlightedText
        : (flat ? QPalette::WindowText : QPalette::ButtonText);
...
bool Style::drawProgressBarContentsControl(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (busy) {
        ...
        const auto second = palette.color(QPalette::Window);
        ...
        const auto contentsColor = palette.color(QPalette::Highlight);
...
bool Style::drawScrollBarSliderControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const bool pressed(enabled && (state & State_Sunken));
    ...
    auto color = _helper->scrollBarHandleColor(palette, mouseOver, hasFocus, opacity, mode, pressed);
    if (StyleConfigData::animationsEnabled() && !pressed) {
...
bool Style::drawFocusFrame(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (targetWidget && targetWidget->inherits("QSlider")) {
        return true;
    }
    ...
    qreal innerRadius = _helper->frameRadius();
...
bool Style::drawRubberBandControl(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    painter->setPen(outline);
    painter->setBrush(background);
    painter->drawRoundedRect(_helper->strokedRect(option->rect), _helper->frameRadius(), _helper->frameRadius());
...
bool Style::drawTabBarTabLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // Suppress the parent focus decoration; tabs use selection and hover only.
    ...
    if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab *>(option)) {
        ...
        NitruxPrivate::PainterStateSaver pss(painter, verticalTabs);
        if (verticalTabs) {
        ...
        const bool inactiveTab = !(tab->state & State_Selected) && !(tab->state & State_MouseOver);
        painter->save();
        if (inactiveTab) {
            painter->setOpacity(0.7);
        }
...
bool Style::drawTabBarTabShapeControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // Focus is not a tab highlight. A status strip is opt-in for tabs that
    // explicitly represent opening or running a command as root.
    bool rootStatus = false;
    if (tabBar && tabOption->tabIndex >= 0) {
        const QVariantMap tabData = tabBar->tabData(tabOption->tabIndex).toMap();
        rootStatus = tabData.value(QStringLiteral("_nitrux_root_status")).toBool()
            || tabData.value(QStringLiteral("nitruxRootStatus")).toBool()
            || tabData.value(QStringLiteral("rootStatus")).toBool()
            || tabData.value(QStringLiteral("openAsRoot")).toBool()
            || tabData.value(QStringLiteral("runningAsRoot")).toBool();
    }
    if (widget) {
        rootStatus = rootStatus || widget->property(PropertyNames::rootStatus).toBool();
    }
    ...
    // Keep neighboring tabs visually separated. The inset is applied only to
    // the painted shape, so Qt/Dolphin tab layouts retain their native geometry.
    const int tabSpacing = Metrics::TabBar_TabSpacing;
    if (verticalTabs) {
        rect.adjust(0, tabSpacing / 2, 0, -(tabSpacing - tabSpacing / 2));
    } else {
        rect.adjust(tabSpacing / 2, 0, -(tabSpacing - tabSpacing / 2), 0);
    }
    ...
    case QTabBar::TriangularNorth:
        if (selected) {
            ...
            // Neighboring tabs remain separated even when one is selected or hovered.
        }
        break;
    ...
    QHash<QByteArray, bool> stateProperties;
    stateProperties["enabled"] = enabled;
    stateProperties["rootStatus"] = rootStatus;
    stateProperties["hovered"] = hovered;
    stateProperties["down"] = down;
...
//______________________________________________________________
bool Style::drawSplitterControl(const QStyleOption *, QPainter *, const QWidget *widget) const
{
    const bool isKatePane = widget
        && (widget->inherits("KateMDI::Sidebar") || widget->inherits("KateMDI::MainWindow")
            || hasParent(widget, "KateMDI::Sidebar") || hasParent(widget, "KateMDI::MainWindow"));
    if (isKatePane) {
        // KateMDI pane splitters remain interactive but do not draw dividers.
        return true;
    }
...
bool Style::drawGroupBoxComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (groupBoxOption->subControls & QStyle::SC_GroupBoxFrame) {
        ...
        NitruxPrivate::PainterStateSaver pss(painter);
    ...
    if ((groupBoxOption->subControls & QStyle::SC_GroupBoxLabel) && !groupBoxOption->text.isEmpty()) {
        NitruxPrivate::PainterStateSaver painterStateSaver(painter);
...
bool Style::drawToolButtonComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    } else if (menuStyle == NitruxPrivate::ToolButtonMenuArrowStyle::InlineSmall || menuStyle == NitruxPrivate::ToolButtonMenuArrowStyle::InlineLarge) {
        copy.rect = menuRect;
        copy.state &= ~State_MouseOver;
        copy.state &= ~State_Sunken;
        copy.state &= ~State_On;
...
bool Style::drawComboBoxComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (option->subControls & SC_ComboBoxArrow) {
        ...
        if (down && enabled && !empty) {
            arrowColor = option->palette.color(QPalette::HighlightedText);
        }
...
bool Style::drawSliderComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (sliderOption->subControls & SC_SliderGroove) {
        // Base track; MauiKit darkens it while the slider is interacted with.
        const auto baseGrooveColor = _helper->alphaColor(palette.color(QPalette::WindowText), 0.2);
        const bool interacting = mouseOver || (state & State_Sunken);
        const auto grooveColor = interacting ? baseGrooveColor.darker(125) : baseGrooveColor;
        ...
        if (!enabled) {
            ...
            if (sliderOption->orientation == Qt::Horizontal) {
                ...
                // Fill: MauiKit uses a slightly translucent highlight segment.
                const auto sliderFill = _helper->alphaColor(highlight, 0.7);
                _helper->renderSliderGroove(painter, upsideDown ? rightRect : leftRect, sliderFill, palette.color(QPalette::Window));
                ...
                // Fill: MauiKit uses a slightly translucent highlight segment.
                const auto sliderFill = _helper->alphaColor(highlight, 0.7);
                _helper->renderSliderGroove(painter, upsideDown ? bottomRect : topRect, sliderFill, palette.color(QPalette::Window));
            }
        }
    }
    ...
    if (sliderOption->subControls & SC_SliderHandle) {
        // MauiKit uses a solid highlight-colored, borderless handle.
        const bool sunken(state & (State_On | State_Sunken));
        const auto handleColor = palette.color(QPalette::Highlight);
        _helper->renderSliderHandle(painter, handleRect, handleColor, QColor(), sunken);
    }
...
bool Style::drawDialComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (sliderOption->subControls & SC_DialHandle) {
        ...
        // render
        _helper->renderSliderHandle(painter, handleRect, background, outline, sunken);
    }
...
{
    if (!option) {
        qCWarning(NITRUX) << "Style::drawScrollBarComplexControl: Style can't draw scrollbar without options";
        return true;
    }
