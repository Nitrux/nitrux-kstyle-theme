...
QRect Style::progressBarLabelRect(const QStyleOption *option, const QWidget *) const
{
    ...
    rect.setLeft(rect.right() - textWidth - 4);
    rect = visualRect(option, rect);
    return rect;
}
...
QRect Style::tabWidgetTabBarRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    if (verticalTabs) {
        ...
        if (tabBarAlignment == Qt::AlignCenter)
            ...
            tabBarRect.moveLeft(rect.left() + (documentMode ? 0 : Metrics::Frame_FrameWidth));
...
bool Style::drawPanelButtonCommandPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (flat) {
        ...
        const QColor outline = hasHighlightNeutral(widget, option, mouseOver) ? _helper->neutralText(palette)
...
bool Style::drawIndicatorCheckBoxPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    QColor color = hasHighlightNeutral(widget, option, mouseOver) ? _helper->neutralText(palette)
                                                                  : _helper->checkBoxIndicatorColor(palette, mouseOver, enabled && active, opacity, mode);
...
bool Style::drawMenuBarEmptyAreaControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (LightlyPrivate::possibleTranslucentToolBars.isEmpty())
...
{
    const auto &palette(option->palette);
    const auto color(_helper->alphaColor(palette.color(QPalette::WindowText), 0.1));
