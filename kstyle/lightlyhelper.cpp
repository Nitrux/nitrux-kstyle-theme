...
{
    QColor color(alphaColor(palette.color(QPalette::WindowText), 0.4));
                               ...
                               bool sunken) const
{
    ...
    if (outline.isValid()) {
        ...
        */
        //<>
        painter->setPen(QPen(outline.darker(hasFocus ? 110 : 103), 1.0));
    ...
    if (color.isValid()) {
        ...
        */
        //<>
        painter->setBrush(color.lighter(hasFocus ? 103 : 101));
...
//______________________________________________________________________________
void Helper::renderTabWidgetFrame(QPainter *painter,
                                  const QRect &rect,
                                  const QColor &color,
                                  Corners corners,
                                  const bool windowActive,
                                  Corners tabBarCorners,
                                  const int tabBarSize) const
{
    painter->setRenderHint(QPainter::Antialiasing);
    ...
    if (tabBarSize > 0) {
        ...
        if ((tabBarCorners & CornerTopLeft) && (tabBarCorners & CornerTopRight)) {
            QRect tabBarRect =
                QRect(QPoint(frameRect.x() - shadowOverlap, frameRect.y() - shadowOverlap), QSize(frameRect.width() + shadowOverlap * 2, tabBarSize + 1));
...
void Helper::renderSelection(QPainter *painter, const QRect &rect, const QColor &color, Corners corners) const
{
    ...
    */
    ...
    QPainterPath path(roundedPath(outlineRect, corners, StyleConfigData::cornerRadius()));
    painter->drawPath(path);
}
        ...
        // mouse over or normal state
            ...
            // normal or mouse over
                ...
                if (mouseOver && !hasFocus) {
                    ...
                    renderOutline(painter, frameRect, radius, 25);
                } else {
...
//______________________________________________________________________________
void Helper::renderCheckBox(QPainter *painter, const QRect &rect, const QColor &color, const QColor &shadow, bool sunken, CheckBoxState state, qreal animation)
    const
{
    // setup painter
    painter->setRenderHint(QPainter::Antialiasing, true);
...
void Helper::renderProgressBarGroove(QPainter *painter, const QRect &rect, const QColor &color) const
{
    ...
    const float antialiasingfix = PenWidth::Frame / 2.0;
    ...
    if (color.isValid()) {
        // painter->setPen(Qt::NoPen);
        QPen pen(color.darker(125), PenWidth::Frame);
        painter->setPen(pen);
        painter->setBrush(color);
        painter->drawRoundedRect(baseRect, radius, radius);
    }
...
void Helper::renderArrow(QPainter *painter, const QRect &rect, const QColor &color, ArrowOrientation orientation) const
{
     ...
     */
    case ArrowUp:
        arrow = QVector<QPointF>{QPointF(-4.5, 1.5), QPointF(0, -3), QPointF(4.5, 1.5), QPointF(-4.5, 1.5)};
        break;
    case ArrowDown:
        arrow = QVector<QPointF>{QPointF(-4.5, -1.5), QPointF(0, 3), QPointF(4.5, -1.5), QPointF(-4.5, -1.5)};
        break;
    case ArrowLeft:
        arrow = QVector<QPointF>{QPointF(1.5, -4.5), QPointF(-3, 0), QPointF(1.5, 4.5), QPointF(1.5, -4.5)};
        break;
    case ArrowRight:
        arrow = QVector<QPointF>{QPointF(-1.5, -4.5), QPointF(3, 0), QPointF(-1.5, 4.5), QPointF(-1.5, -4.5)};
        break;
    case ArrowDown_Small:
        arrow = QVector<QPointF>{QPointF(1.5, 3.5), QPointF(3.5, 5.5), QPointF(5.5, 3.5), QPointF(1.5, 3.5)};
        break;
    default:
        break;
    }
...
void Helper::renderRoundedRectShadow(QPainter *painter, const QRectF &rect, const QColor &color, qreal radius) const
{
     ...
     */
    // Right side
    QRegion clip(rect.right() - std::ceil(radius), rect.top(), std::ceil(radius) + PenWidth::Shadow, rect.height());
    // Bottom side
    clip = clip.united(QRegion(rect.left(), rect.bottom() - std::ceil(radius), rect.width(), std::ceil(radius) + PenWidth::Shadow));
